...
public:
	...
	Light l;
	Model model;
	GameObject(char* name, char* modelName, char* vShaderName, char* fShaderName, Light light = Light(glm::vec3(-99.0f,-99.0f,-99.0f))){
	...
	void translate(glm::vec3 target){
		this->lastPos = target;
		this->transform = tr_translate(this->transform, target);
	}
};
#endif
